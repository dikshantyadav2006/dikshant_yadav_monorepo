...
const DepthCarousel = forwardRef(function DepthCarousel(
  ...
  useImperativeHandle(
    ref,
    () => ({
      setFocus: (index, animate = true) => setFocus(index, animate),
      navigateBy,
      getActive: () => focusRef.current
    }),
    [setFocus, navigateBy]
  );
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          <div
            key={i}
            className="absolute left-1/2 top-1/2 cursor-pointer overflow-hidden bg-[#0b0d12] border-b-[3px] border-r-[3px] border-[var(--dark-color)] dark:border-[var(--light-color)] shadow-[0_30px_60px_-20px_rgba(0,0,0,0.65),0_8px_20px_-10px_rgba(0,0,0,0.5)] [transform:translate(-50%,-50%)] [transform-origin:center] [will-change:transform,opacity,filter]"

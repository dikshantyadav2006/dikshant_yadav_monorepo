...
/**
 * The automatic JSX runtime hands the key to the element factory as a *third*
 * argument — `jsx(type, props, key)` — whereas `React.createElement(type,
 * props)` reads `key` from inside props and treats any further positional
 * arguments as children. Binding the autogenerated imports to `createElement`
 * would drop authored `key={…}` expressions (and re-interpret the key value as
 * a stray child), triggering "Each child in a list should have a unique key
 * prop" warnings. It also re-enables React's legacy child-key validation for
 * statically-authored multi-children arrays, which the real automatic runtime
 * deliberately skips. So we bind directly to React's own `jsx`/`jsxs` factories.
 */
function resolveJsxRuntime(imported) {
  if (imported === 'jsxs') return runtimeJsxs;
  return runtimeJsx; // 'jsx' and 'jsxDEV' both operate on (type, props, key)
}
      ...
      if (JSX_RUNTIME_MODULES[moduleName]) {
        for (const spec of splitSpecs(named[1])) {
          ...
          if (imported === 'jsx' || imported === 'jsxs' || imported === 'jsxDEV') {
            bindings.push({ name: local, value: resolveJsxRuntime(imported) });

...
import type { WorkVersion } from '@dikshant/types';
import { useWorkBuilderStore } from '../../features/work-builder/store';
import { getWorkVersions, restoreWorkVersion } from '../../features/work-builder/api';

interface WorkVersionsSidebarProps {
  workId: string;
  onClose: () => void;
  onRestoreSuccess: (restoredData: any) => void;
}

export function WorkVersionsSidebar({ workId, onClose, onRestoreSuccess }: WorkVersionsSidebarProps) {
  const [versions, setVersions] = useState<
    Pick<WorkVersion, 'id' | 'workId' | 'version' | 'changeLabel' | 'savedById' | 'createdAt'>[]
  >([]);
  ...
  const loadVersions = async () => {
    ...
    setError('');
    try {
      const data = await getWorkVersions(workId);
      setVersions(data || []);
    } catch (err) {
  ...
  const handleRestore = async (version: number) => {
    ...
    setError('');
    try {
      const res = await restoreWorkVersion(workId, version);
...
export default WorkVersionsSidebar;

...
/**
 * The automatic JSX runtime hands `key` to the element factory as a *third*
 * argument (`jsx(type, props, key)`), while `React.createElement` reads it from
 * props and treats any extra positional argument as a child. Binding the
 * autogenerated imports to `createElement` drops authored `key={…}` (and leaks
 * the key value as a stray child), producing "Each child in a list should have
 * a unique key prop" warnings; it also re-enables legacy child-key validation
 * on statically-authored multi-children arrays. Binding to React's own
 * `jsx`/`jsxs` factories preserves the automatic-runtime contract exactly.
 */
function resolveJsxRuntime(imported: string): typeof runtimeJsx {

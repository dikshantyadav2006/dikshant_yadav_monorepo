...
async function getTagPosts(slug: string) {
  try {
    const res = await fetch(`${API_URL}/posts?tagSlug=${slug}&limit=12`, { next: { revalidate: 30 } });
    ...
    <div className="space-y-8 py-4">
      ...
      <div className="border-b border-border/40 pb-6">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Viewing posts published containing the tag &ldquo;#{slug}&rdquo;.
        </p>
      </div>
...
export async function generateMetadata({
  ...
  const { slug } = await params;
  return {
    title: `#${slug} Articles | Dikshant Yadav`,
    description: `Browse all developer blog posts matching the #${slug} tag.`,
  };
}
